...
describe('validation utilities', () => {
  ...
  describe('validateMessage', () => {
    ...
    describe('warnings', () => {
      ...
      it('should warn about GitHub tokens', () => {
        const result = validateMessage('token: ghp_1234567890abcdefghijklmnopqrstuvwxyz1234');
        expect(result.valid).toBe(true);
        expect(result.warnings).toBeDefined();
      });
  ...
  describe('validateServerConfig', () => {
    ...
    describe('name validation', () => {
      ...
      it('should reject name starting with number', () => {
        const result = validateServerConfig({ name: '1server', command: 'node' });
        expect(result.valid).toBe(false);
        expect(result.error).toContain('must start with a letter');
      });

      it('should reject name with special characters', () => {
        const result = validateServerConfig({ name: 'my.server', command: 'node' });
        expect(result.valid).toBe(false);
      });

      it('should reject name exceeding 50 characters', () => {
        const longName = 'a'.repeat(51);
        const result = validateServerConfig({ name: longName, command: 'node' });
        expect(result.valid).toBe(false);
    ...
    describe('warnings', () => {
      it('should warn about absolute path commands', () => {
        ...
        expect(result.valid).toBe(true);
        expect(result.warnings).toContain('Using absolute path for command - ensure it exists on the system');
      });
    });
  });
    ...
    describe('warnings for overly permissive patterns', () => {
      it('should warn about wildcard-only patterns', () => {
        ...
        for (const pattern of patterns) {
          ...
          expect(result.valid).toBe(true);
          expect(result.warnings).toContain('This pattern allows all paths - consider being more specific');
        }
      });
      ...
      it('should warn about patterns without directory', () => {
        ...
        expect(result.valid).toBe(true);
        expect(result.warnings).toContain('Pattern does not include a directory - may match more than intended');
      });
    });
  });
    ...
    describe('object validation', () => {
      ...
      it('should validate property types', () => {
        ...
        expect(validateJsonSchema(
          { name: 'John', age: 30, active: true, tags: [] },
          schema
        ).valid).toBe(true);
  ...
  describe('combineValidationResults', () => {
    ...
    it('should include warnings from failed validations', () => {
      const result = combineValidationResults(
        { valid: false, error: 'Error', warnings: ['Warning'] }
      );
      expect(result.valid).toBe(false);
