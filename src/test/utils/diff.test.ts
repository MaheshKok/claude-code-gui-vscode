...
describe('diff utilities', () => {
  ...
  describe('computeLineDiff', () => {
    ...
    describe('options', () => {
      it('should ignore whitespace when ignoreWhitespace is true', () => {
        const oldContent = 'line 1\n  indented  \nline 3';
        const newContent = 'line 1\nindented\nline 3';
        const result = computeLineDiff(oldContent, newContent, { ignoreWhitespace: true });
      ...
      it('should detect whitespace changes when ignoreWhitespace is false', () => {
        const oldContent = 'line 1\n  indented  \nline 3';
        const newContent = 'line 1\nindented\nline 3';
        const result = computeLineDiff(oldContent, newContent, { ignoreWhitespace: false });
  ...
  describe('computeContextualDiff', () => {
    ...
    it('should include context lines around changes', () => {
      const oldContent = 'a\nb\nc\nd\ne\nf\ng\nh\ni\nj';
      const newContent = 'a\nb\nc\nd\nX\nf\ng\nh\ni\nj';
      const result = computeContextualDiff(oldContent, newContent, { contextLines: 2 });
    ...
    it('should add separators for non-adjacent changes', () => {
      const oldContent = 'a\nb\nc\nd\ne\nf\ng\nh\ni\nj';
      const newContent = 'X\nb\nc\nd\ne\nf\ng\nh\ni\nY';
      const result = computeContextualDiff(oldContent, newContent, { contextLines: 1 });
    ...
    it('should handle contextLines of 0', () => {
      const oldContent = 'a\nb\nc';
      const newContent = 'a\nX\nc';
      const result = computeContextualDiff(oldContent, newContent, { contextLines: 0 });
  ...
  describe('formatDiffHtml', () => {
    ...
    describe('basic output', () => {
      ...
      it('should escape HTML in content', () => {
        const diff: DiffResult = {
          lines: [
            { type: 'equal', content: '<script>alert("xss")</script>', oldLineNumber: 1, newLineNumber: 1 },
          ],
          additions: 0,
          deletions: 0,
          unchanged: 1,
    ...
    describe('options', () => {
      ...
      it('should wrap long lines when wrapLines is true', () => {
        ...
        const diff: DiffResult = {
          lines: [{ type: 'equal', content: longLine, oldLineNumber: 1, newLineNumber: 1 }],
          additions: 0,
          deletions: 0,
          unchanged: 1,
          isIdentical: true,
        };
        const html = formatDiffHtml(diff, { wrapLines: true, maxLineWidth: 50 });
