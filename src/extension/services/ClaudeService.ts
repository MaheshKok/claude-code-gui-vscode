...
export class ClaudeService implements vscode.Disposable {
  ...
  public async sendMessage(
    ...
    if (options.mcpConfigPath) {
      args.push(
        "--mcp-config",
        this._convertToWSLPath(options.mcpConfigPath, config),
      );
    }
    ...
    if (claudeProcess.stdout) {
      claudeProcess.stdout.on("data", (data) => {
        ...
        console.log(
          "[ClaudeService] Received stdout chunk:",
        ...
        for (const line of lines) {
          if (line.trim()) {
            try {
              ...
              console.log(
                "[ClaudeService] Parsed JSON message type:",
                jsonData.type,
              );
  ...
  private _handleControlRequest(controlRequest: any): void {
    ...
    console.log(
      `Permission request for tool: ${toolName}, requestId: ${requestId}`,
    );
