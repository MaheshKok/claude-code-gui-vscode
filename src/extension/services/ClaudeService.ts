...
export class ClaudeService implements vscode.Disposable {
                ...
                for (const line of lines) {
                    if (line.trim()) {
                        try {
                            ...
                            console.log('[ClaudeService] Failed to parse JSON line:', line.substring(0, 100), error);
                        }
                    }
                }
            });
    ...
    public async stopProcess(): Promise<void> {
        ...
        try {
            ...
            if (this._isWslProcess && process.platform === 'win32') {
                ...
                try {
                    cp.execSync(`taskkill /pid ${this._process.pid} /T /F`, { stdio: 'ignore' });
                } catch {
                    // Ignore errors
                }
    ...
    private _processJsonData(jsonData: any, claudeProcess: cp.ChildProcess): void {
        console.log('[ClaudeService] Processing message type:', jsonData.type, 'subtype:', jsonData.subtype);
