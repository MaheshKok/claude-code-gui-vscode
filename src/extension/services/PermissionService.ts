...
export class PermissionService implements vscode.Disposable {
    ...
    public async getPermissions(): Promise<Permissions> {
        try {
            ...
            const permissionsUri = vscode.Uri.file(path.join(this._permissionsPath, 'permissions.json'));
    ...
    public async removePermission(
        ...
        try {
            ...
            if (command === null) {
                ...
                if (Array.isArray(permissions.alwaysAllow[toolName])) {
                    permissions.alwaysAllow[toolName] = (permissions.alwaysAllow[toolName] as string[]).filter(
